...
import { ArrowLeft, Plus, Trash2, X, Save, Upload, ScanText, Loader2, FileText, RefreshCw, CheckCircle2, AlertTriangle } from 'lucide-react'
...
export default function EquipamentoDetalhePage() {
  ...
  const [processandoPdf,  setProcessandoPdf]  = useState<string | null>(null) // certPath sendo processado
  const [processoLog,     setProcessoLog]     = useState<string[]>([])
  const [modalCert,    setModalCert]    = useState(false)
  const [grupos,       setGrupos]       = useState<{id:string;nome:string;cor:string;subgrupos:{id:string;nome:string;numero:string}[]}[]>([])

  useEffect(() => {
    fetch('/api/grupos').then(r=>r.json()).then(g => setGrupos(Array.isArray(g)?g:[])).catch(()=>{})
  }, [])

  async function salvarClassificacao(grupoId: string, subgrupoId: string) {
    await fetch(`/api/equipamentos/${id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({grupoId,subgrupoId}) })
    carregarEquip()
  }
  ...
  async function processarCertsPendentes() {
    if (!equip?.certsPendentes?.length) return
    const api = (window as any).electronAPI
    if (!api?.lerPdf) { alert('Disponível apenas no aplicativo (Electron).'); return }
    const pendentes = [...(equip.certsPendentes || [])]
    setProcessoLog([])
    const log: string[] = []
    for (const certPath of pendentes) {
      setProcessandoPdf(certPath)
      try {
        const res = await api.lerPdf(certPath)
        if (!res.ok) { log.push(`✗ ${certPath.split(/[\\/]/).pop()} — ${res.error}`); continue }
        const folder = equip.tag
        const r = await fetch('/api/equipamentos/importar-lote', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ itens: [{ folder, certPath, text: res.text, items: res.items }] }),
        })
        const rel = await r.json()
        if (!r.ok) { log.push(`✗ ${certPath.split(/[\\/]/).pop()} — ${rel.error}`); continue }
        if (rel.sucessos?.length || rel.atualizados?.length)
          log.push(`✓ ${certPath.split(/[\\/]/).pop()}`)
        else if (rel.pulados?.length)
          log.push(`! ${certPath.split(/[\\/]/).pop()} — ${rel.pulados[0]?.motivo}`)
        else
          log.push(`? ${certPath.split(/[\\/]/).pop()}`)
      } catch (e) {
        log.push(`✗ ${certPath.split(/[\\/]/).pop()} — ${String(e)}`)
      }
    }
    setProcessandoPdf(null)
    setProcessoLog(log)
    // Recarrega equipamento e certificados
    carregarEquip()
    ...
    <div>
        ...
        <div className="card p-5">
          ...
          <div className="flex gap-5 items-start flex-wrap">
            <div className="grid grid-cols-2 gap-4 text-sm flex-1 min-w-[260px]">
              <div>
                <p className="text-[9px] font-mono tracking-[2px] uppercase text-white/30 mb-0.5">Status de calibração</p>
                <StatusPill status={equip.status}/>
              </div>
          ...
          {/* Classificação inline — grupo e subgrupo */}
          {grupos.length > 0 && (
            <div className="mt-5 pt-5 border-t border-white/6">
              <p className="text-[9px] font-mono tracking-[2px] uppercase text-white/30 mb-3">Classificação</p>
              <div className="flex items-center gap-3 flex-wrap">
                <div>
                  <p className="text-[9px] font-mono uppercase text-white/30 mb-1">Grupo</p>
                  <select className="input text-[12px] py-1.5 h-auto"
                    value={equip.grupoId}
                    onChange={e => {
                      const g = grupos.find(x => x.id === e.target.value)
                      const sub = g?.subgrupos[0]?.id ?? ''
                      salvarClassificacao(e.target.value, sub)
                    }}>
                    {grupos.map(g => <option key={g.id} value={g.id}>{g.nome}</option>)}
                  </select>
                </div>
                <div>
                  <p className="text-[9px] font-mono uppercase text-white/30 mb-1">Subgrupo</p>
                  <select className="input text-[12px] py-1.5 h-auto"
                    value={equip.subgrupoId}
                    onChange={e => salvarClassificacao(equip.grupoId, e.target.value)}>
                    {(grupos.find(g => g.id === equip.grupoId)?.subgrupos ?? []).map(s => (
      ...
      {tab==='certificados' && (
        <div>
          ...
          {/* Certificados pendentes da rede */}
          {equip.certsPendentes && equip.certsPendentes.length > 0 && (
            <div className="card p-4 mb-4 border-amber-500/20 bg-amber-500/5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-semibold text-amber-400 text-[13px] flex items-center gap-1.5 mb-1">
                    <RefreshCw size={13}/> {equip.certsPendentes.length} PDF(s) da rede aguardando processamento
                  </p>
                  <div className="space-y-0.5">
                    {equip.certsPendentes.map((p, i) => (
                      <p key={i} className="text-[10px] font-mono text-white/35 truncate max-w-[480px]">
                        {p.split(/[\\/]/).pop()}
                      </p>
                    ))}
                  </div>
                </div>
                <button type="button" onClick={processarCertsPendentes}
                  disabled={!!processandoPdf}
                  className="btn-secondary flex-shrink-0 text-xs">
                  {processandoPdf
                    ? <><Loader2 size={12} className="animate-spin"/> Processando…</>
                    : <><ScanText size={12}/> Processar PDFs</>}
                </button>
              </div>
              {processoLog.length > 0 && (
                <div className="mt-3 space-y-0.5 border-t border-white/8 pt-3">
                  {processoLog.map((l, i) => (
                    <p key={i} className={`text-[11px] font-mono ${l.startsWith('✓') ? 'text-green-400' : l.startsWith('!') ? 'text-amber-400' : 'text-red-400'}`}>{l}</p>
                  ))}
                </div>
              )}
            </div>
