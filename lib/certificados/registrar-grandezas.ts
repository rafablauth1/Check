...
export function grandezasDoCertificado(cert: Certificado): GrandezaMetrologica[] {
  ...
  const add = (nome?: string, unidade?: string, val?: number, inc?: number) => {
    const n = (nome || '').trim()
    if (!n) return
    // Descarta nomes claramente errados vindos de OCR
    if (n.length > 60) return
    if (/\d{2}\/\d{2}\/\d{4}/.test(n)) return   // data DD/MM/YYYY no nome
    if (/\d{4}-\d{2}-\d{2}/.test(n)) return      // data ISO no nome
    if (n.includes(':') && n.length > 30) return  // parece frase longa (ex: "Data de emissão do cert.: ...")
    if (/^[\d\s.,±+-]+$/.test(n)) return          // só números/sinais → não é nome de grandeza
